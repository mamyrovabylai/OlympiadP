...
<input type="submit" value="Выбрать" class="buttonSelect" name="button"/>
</form>
<?php
$mysqli=connect();
...
if(isset($_POST["subject"]) && isset($_POST["button"]) ){
	...
	$q="SELECT `name` FROM `olympiads` WHERE `time_end`<'$now' AND `subject`='$subject';";
	}else {
	$q="SELECT `name` FROM `olympiads` WHERE `time_end`<'$now';"; 
	}
